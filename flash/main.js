...
function populate() {
	...
	for (var i = 0, arc = null; i < 4; i++) {
		...
		part.add(new _base.Line([x + 75, y - 160 + i * lineWidth * 3], [x - 60, y - 30 + i * lineWidth * 3], sColor, lineWidth, startTime, duration));
	}
	part.add(new _base.ClearRect(x + 73, y - 161, lineWidth, lineWidth * 12, startTime - duration));
	part.add(new _base.ClearRect(x - 59 - lineWidth, y - 30 - lineWidth, lineWidth, lineWidth * 12, startTime - duration));
	parts.push(part);
	...
	for (var _i = 0, _arc = null; _i < 4; _i++) {
		startTime = startTime + duration / 4;
		part.add(new _base.Line([x - 35 - _i * lineWidth * 2, y - 19 + _i * lineWidth * 2], [x - 35 - _i * lineWidth * 2 + 97, y - 19 + _i * lineWidth * 2], sColor, lineWidth, startTime, duration));
	}
	...
	for (var _i2 = 0, _arc2 = null; _i2 < 4; _i2++) {
		startTime = startTime + duration / 4;
		part.add(new _base.Line([x + 65, y - 20 + _i2 * lineWidth * 3], [x - 80, y + 120 + _i2 * lineWidth * 3], sColor, lineWidth, startTime, duration));
	}
	part.add(new _base.ClearRect(x + 62, y - 21, lineWidth, lineWidth * 12, startTime - duration));
	part.add(new _base.ClearRect(x - 79 - lineWidth, y + 120 - lineWidth, lineWidth, lineWidth * 12, startTime - duration));
	parts.push(part);
	...
	for (var _i3 = 0, _arc3 = null; _i3 < 4; _i3++) {
		startTime = startTime + duration / 4;
		_arc3 = new _base.Arc([x, y], 133 - _i3 * lineWidth * 2, -(Math.PI / 3 + Math.PI / 15), -2 * Math.PI, circleColor, lineWidth, startTime, duration);
		_arc3.anticlockwise = true;
		part.add(_arc3);
	}

	part.add(new _base.ClearQuad(x + 75, y - 160 + lineWidth * 4, x - 5, y - 160 + lineWidth * 24, lineWidth * 12, startTime - duration));
	part.add(new _base.ClearQuad(x + 15, y + 30 + lineWidth * 4, x - 75, y + 35 + lineWidth * 24, lineWidth * 12, startTime - duration));
