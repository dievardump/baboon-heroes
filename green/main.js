...
function populate() {
	...
	var part = new _base.Part(width, height),
	    arc = new _base.Arc([x, y], 145, -Math.PI / 2, -2 * Math.PI, '#fff', lineWidth, startTime, duration);
	...
	for (var i = 0, _arc = null; i < 4; i++) {
		...
		part.add(new _base.Line([x - 77, y - 77 + i * lineWidth * 2], [x + 77, y - 77 + i * lineWidth * 2], color, lineWidth, startTime, duration));
	}
	...
	for (var _i = 0, _arc2 = null; _i < 4; _i++) {
		startTime = startTime + duration / 4;
		part.add(new _base.Line([x + 77, y + 77 - _i * lineWidth * 2], [x - 77, y + 77 - _i * lineWidth * 2], color, lineWidth, startTime, duration));
	}
	...
	for (var _i2 = 0, _arc3 = null; _i2 < 4; _i2++) {
		startTime = startTime + duration / 4;
		_arc3 = new _base.Arc([x, y], 77 - _i2 * lineWidth * 2, -Math.PI / 2, -2 * Math.PI, color, lineWidth, startTime, duration);
		_arc3.anticlockwise = true;
		part.add(_arc3);
	}

	part.add(new _base.ClearRect(x - 77, y - 80, 145, 27, startTime - duration));
	part.add(new _base.ClearRect(x - 77, y + 53, 145, 27, startTime - duration));
